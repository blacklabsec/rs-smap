...
async fn scan_targets(
    ...
    for ip in targets {
        ...
        let task = tokio::spawn(async move {
            ...
            // Correlate services and OS
            let port_numbers: Vec<u16> = result.ports.iter().map(|p| p.number).collect();
            let cpes: Vec<String> = result
                .ports
                .iter()
                .filter_map(|p| p.service.as_ref())
                .flat_map(|s| s.cpes.clone())
                .collect();

            let (port_infos, os_correlation) = correlate(&port_numbers, &cpes);
            ...
            for (i, port_info) in port_infos.iter().enumerate() {
                if let Some(port) = result.ports.get_mut(i) {
                    if let Some(ref service) = port.service {
                        ...
                        if updated_service.name == "unknown" && !port_info.service.is_empty() {
                            updated_service.name = port_info.service.clone();
                        }
                        port.service = Some(updated_service);
                    }
                }
            }
