...
use crate::output::common::{
    format_time, get_command, pad_left, pad_right, OutputWriter, TimeFormat,
};
...
impl NmapFormatter {
    ...
    pub fn write_result(&mut self, result: &ScanResult) -> io::Result<()> {
        ...
        output.push_str(&format!(
            ...
            pad_left("", longest_port - 4 + 1),
            pad_left("", longest_service - 7 + 1 + 1)
        ));
        ...
        for port in &result.ports {
            ...
            output.push_str(&format!(
                "{}{}  {}{}\n",
                port_str,
                pad_left("open", longest_port - port_str.len() + 1),
