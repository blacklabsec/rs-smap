...
//! ];
//!
//! let (result_ports, os_info, uncorrelated_cpes) = correlate(&ports, &cpes);
...
/// # Returns
///
/// A tuple of (port_info_list, os_info, uncorrelated_cpes)
///
/// # Example
///
/// ```
...
/// ];
///
/// let (port_results, os, uncorrelated) = correlate(&ports, &cpes);
/// assert_eq!(port_results.len(), 2);
/// ```
pub fn correlate(ports: &[u16], cpes: &[String]) -> (Vec<PortInfo>, OsCorrelation, Vec<String>) {
...
mod tests {
    ...
    fn test_specific_cpe_scoring() {
        ...
        let (result, _, _) = correlate(&ports, &[specific_cpe]);
    ...
    fn test_cpe_application_filter() {
        ...
        let (result, _, _) = correlate(&ports, &os_only_cpe);
